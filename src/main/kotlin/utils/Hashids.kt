package com.egorsmkv.short_urls_service.utils
...
class Hashids(
    ...
    fun encode(vararg numbers: Long): String = when {
        ...
        numbers.any { it > maxNumber } -> throw IllegalArgumentException("Number can not be greater than ${maxNumber}L")
            ...
            val (encodedString, encodingAlphabet) = initialEncode(
                numbers.asList(),
                finalSeparators.toCharArray(),
                initialCharacter.toString(),
                0,
                finalAlphabet,
                initialCharacter.toString()
            )
    ...
    private fun calculateSeparatorsLength(alphabet: String): Int =
        when (val s = ceil(alphabet.length / separatorDiv).toInt()) {
            1 -> 2
            else -> s
        }
    ...
    private tailrec fun doUnhash(
        ...
        when {
            currentIndex < input.size -> {
                ...
                val newNumber =
                    currentNumber + (position * alphabetLengthDouble.pow((input.size - currentIndex - 1))).toLong()
    ...
    private tailrec fun shuffle(data: ShuffleData, currentPosition: Int, limit: Int): ShuffleData = when {
        currentPosition < limit -> data
            ...
            shuffle(
                ShuffleData(currentAlphabet.toList(), data.salt, cumulativeValue, saltReminder + 1),
                currentPosition - 1,
                limit
            )
        }
    ...
    private tailrec fun ensureMinimalLength(halfLength: Int, alphabet: String, returnString: String): String = when {
        returnString.length < finalHashLength -> {
            ...
            val tempReturnString =
                newAlphabet.substring(halfLength) + returnString + newAlphabet.substring(0, halfLength)
